...
def query_ssh_config(hosts, root):
    out = ['## begin hosts generated by terraform.py ##']
    group_vars = yaml.load(open(os.path.join(root, 'group_vars/all.yml'), 'r'))
    ssh_user = None
    template = 'Host {}\n  HostName {}\n  StrictHostKeyChecking no\n  UserKnownHostsFile /dev/null'
    if group_vars.has_key('ansible_ssh_user'):
        ssh_user = group_vars['ansible_ssh_user']
        template = template + '\n  User {}\n'
    out.extend(
        template.format(name, attrs['ansible_ssh_host'], ssh_user)
            for name, attrs, _ in hosts
    )
    out.append('## end hosts generated by terraform.py ##')
    return '\n'.join(out)
...
def main():
    ...
    modes.add_argument('--sshconfig',
                       action='store_true',
                       help='print hosts as a ssh config snippet')
    ...
    if args.list:
        ...
        output = query_ssh_config(hosts, args.root)
        print(output)
